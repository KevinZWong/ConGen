...
import torch
#from ImageP.ImageGenerationOOP import ImageCreator
import subprocess
import threading
...
class ConcurrentImageProcessing:
    ...
    def run_image_generation_command(self, gpu_num, prompt, output):
        print(f"Generating image {output}, gpu_num = {gpu_num}")

        command = ["python3", self.generation_path, "--gpu_number", str(gpu_num), "--prompt", prompt, "--output_file", output]
    ...
    def generate_images(self, image_descriptions, imageFilePath):
        ...
        while len(pooled_results) != len(image_descriptions):
                if any(gpu_status) and images_to_generated != 0:
                    ...
                    futures.append(executor.submit(self.run_image_generation_command, gpu_avaible, image_descriptions[current_image_index], image_name))
